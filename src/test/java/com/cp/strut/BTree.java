...
/**
 * 每个叶结点具有相同的深度。树的高度h 树的度为t 每个结点的关键字 非根至少t-1个，至多2t-1 个,根至少1个
 * 非根非叶子结点最少t子女，最多2t个子女 根至少2两个子女,最多2t个子女
 * 
 */
public class BTree {
...
/*		for (int key : keys) {
	 ...
	 * 想办法从其他地方弄关键字过来，使得该结点的关键字数至少为t。 考虑从相邻结点弄，如果相邻结点有的话，经过父结点进行周转。如果没有，
	...
	public BTreeNode configKeys(BTreeNode node) {
		if (node.keys.size() <= BTree.M - 1) {
			...
			if (nodeRight != null && nodeRight.keys.size() > BTree.M - 1) {
				...
				f.keys.add(node.position - 1,
						nodeLeft.keys.get(nodeLeft.keys.size() - 1));
